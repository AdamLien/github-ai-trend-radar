...
const copy = {
  zh: {
    ...
    count: (shown, total) => `${shown} / ${total} repos`, repo: 'Repository', purpose: '用途', delta: (period) => `${period} 星星增量`, stars: '總星數', trend: '期間走勢', risk: '風險／品質',
  ...
  en: {
    ...
    count: (shown, total) => `${shown} / ${total} repos`, repo: 'Repository', purpose: 'Purpose', delta: (period) => `${period} star delta`, stars: 'Total stars', trend: 'Period trend', risk: 'Risk / quality',
...
function TimelineChart({ row, rangeLabel, onClose, language }) {
...
function App() {
  ...
  const [customStart, setCustomStart] = useState(data.historyDates.at(-7) || data.historyDates[0]); const [customEnd, setCustomEnd] = useState(data.historyDates.at(-1)); const [selectedName, setSelectedName] = useState(data.records[0].name); const [detailOpen, setDetailOpen] = useState(false)
  ...
  const toggleTag = (tag) => setTags((current) => current.includes(tag) ? current.filter((item) => item !== tag) : [...current, tag]); const choosePeriod = (value) => { setPeriod(value); if (value !== 'custom') setCustomEnd(data.historyDates.at(-1)) }; const openDetail = (name) => { setSelectedName(name); setDetailOpen(true) }
  ...
  return <main className="app-shell"><aside className="sidebar"><div className="radar-mark" aria-label="GitHub AI Trend Radar"><span /><span /><b>↗</b></div><div className="side-status"><strong>Radar</strong><span>{data.updatedAt}</span></div><p className="source">{t.sourceShort}</p></aside><section className="workspace" id="radar"><header><div><h1>GitHub AI Trend Radar</h1><p>{t.subtitle}</p></div><div className="header-tools"><div className="language-switch" aria-label={t.language}><button className={language === 'zh' ? 'active' : ''} onClick={() => setLanguage('zh')}>繁中</button><button className={language === 'en' ? 'active' : ''} onClick={() => setLanguage('en')}>EN</button></div><div className="updated">{t.updated}<strong>{data.updatedAt}</strong></div></div></header>
    ...
    <section className="dashboard-grid"><section className="table-panel"><div className="panel-heading"><div><h2>{t.momentum(rangeLabel)}</h2><p>{t.coverage(coverageStart, coverageEnd)}</p></div><span className="result-count">{t.count(filtered.length, data.repoCount)}</span></div><div className="table-wrap"><table><thead><tr><th>#</th><th>{t.repo}</th><th>{t.purpose}</th><th>{t.delta(rangeLabel)}</th><th>{t.stars}</th><th>{t.trend}</th><th>{t.risk}</th></tr></thead><tbody>{filtered.slice(0, 12).map((row, index) => <tr key={row.name} className={selected.name === row.name ? 'selected' : ''} onClick={() => openDetail(row.name)}><td className="rank">{index + 1}</td><td><strong>{row.name}</strong><div className="row-tags">{row.tags.slice(0, 3).map((tag) => <span key={tag} className={`tag ${tag.toLowerCase()}`}>{tagText[tag]?.[language] || tag}</span>)}</div></td><td className="description"><span>{language === 'zh' ? row.descriptionZh : row.description || t.noDescription}</span>{language === 'zh' && <small>{t.original}</small>}</td><td className="delta">+{number(language).format(row.windowDelta)}</td><td>{number(language).format(row.stars)}</td><td><Sparkline series={row.windowSeries} /></td><td><span className={row.category === 'Watch' ? 'risk warning' : 'risk'}>{row.category === 'Watch' ? t.review : row.license}</span></td></tr>)}</tbody></table></div></section><section className="right-rail"><InsightPanel records={filtered.length ? filtered : decorated} tagOptions={tagOptions} language={language} /><TrendChart records={filtered.length ? filtered : decorated} rangeLabel={rangeLabel} language={language} /><div className="category-panel"><div className="panel-heading"><h2>{t.categoryDistribution}</h2><span>{t.byRepos}</span></div>{categoryCounts.map((item) => <div className="distribution" key={item.key}><span>{categoryText[item.key][language]}</span><div><i style={{ width: `${item.count / maxCount * 100}%` }} /></div><b>{item.count}</b></div>)}</div></section></section>
    {detailOpen && <div className="detail-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setDetailOpen(false) }}><div className="detail-modal" role="dialog" aria-modal="true" aria-label={`${selected.name} ${t.timeline}`}><TimelineChart row={selected} rangeLabel={rangeLabel} onClose={() => setDetailOpen(false)} language={language} /></div></div>}<footer>{t.source}</footer></section></main>
}
