...
const copy = {
  zh: {
    ...
    source: '資料來源：GitHub API snapshots 與範圍內 Trending daily；Trending 今日星數不納入時序 delta。', review: '需審核', newEntry: '新進榜', trendingToday: (stars) => `Trending 今日 +${stars}`, noDescription: '尚無用途說明。', original: 'GitHub 專案摘要', periodLabels: ['本日', '3 日', '7 日', '1 個月', '3 個月', '半年', '1 年'],
  ...
  en: {
    ...
    source: 'Source: GitHub API snapshots plus in-scope Trending daily. Trending daily stars are excluded from time-series deltas.', review: 'Review needed', newEntry: 'New entry', trendingToday: (stars) => `Trending today +${stars}`, noDescription: 'No purpose description available.', original: 'GitHub repository summary', periodLabels: ['Today', '3 days', '7 days', '1 month', '3 months', '6 months', '1 year'],
...
function ProjectMark() {
  return <div className="project-mark" aria-hidden="true"><svg viewBox="0 0 48 48"><circle cx="24" cy="24" r="20" /><circle cx="24" cy="24" r="14" /><path d="M24 24 38 10" /><path d="M24 24 34 34" /><circle className="project-mark-signal" cx="38" cy="10" r="3" /></svg><img src={`${import.meta.env.BASE_URL}metabiz-gold-icon-transparent.png`} alt="" /></div>
}
...
function App() {
  const [language, setLanguage] = useState('zh'); const [search, setSearch] = useState(''); const [category, setCategory] = useState('All'); const [tags, setTags] = useState([]); const [sort, setSort] = useState('momentum'); const [period, setPeriod] = useState(7); const [page, setPage] = useState(1)
  ...
  const pageCount = Math.max(1, Math.ceil(filtered.length / pageSize)); const currentPage = Math.min(page, pageCount); const pageRows = filtered.slice((currentPage - 1) * pageSize, currentPage * pageSize)
  ...
  const resetPage = () => setPage(1); const toggleTag = (tag) => { setTags((current) => current.includes(tag) ? current.filter((item) => item !== tag) : [...current, tag]); resetPage() }; const choosePeriod = (value) => { setPeriod(value); if (value !== 'custom') setCustomEnd(data.historyDates.at(-1)); resetPage() }; const openDetail = (name) => { const row = decorated.find((item) => item.name === name); setSelectedName(name); setDetailOpen(true); if (row) loadReadme(row) }
  useEffect(() => { const closeOnEscape = (event) => { if (event.key === 'Escape') setDetailOpen(false) }; window.addEventListener('keydown', closeOnEscape); return () => window.removeEventListener('keydown', closeOnEscape) }, []); useEffect(() => { document.documentElement.lang = language === 'zh' ? 'zh-Hant' : 'en' }, [language]); useEffect(() => { setPage(1) }, [search, category, sort, customStart, customEnd])
  return <main className="app-shell"><a className="skip-link" href="#radar">Skip to radar</a><aside className="sidebar"><div className="project-lockup" aria-label="metabiz GitHub AI Trend Radar"><ProjectMark /><div><strong><i>metabiz</i> / GITHUB AI</strong><b>TREND RADAR</b><span>OPEN-SOURCE INTELLIGENCE</span></div></div><div className="side-status"><strong>Daily signal board</strong><span>{data.repoCount} REPOS · {data.historyDates.length} SNAPSHOTS</span></div><p className="source">{t.sourceShort}</p></aside><section className="workspace" id="radar"><header><div className="hero-copy"><span className="eyebrow">metabiz intelligence / OPEN-SOURCE SIGNAL</span><h1>GitHub AI <em>Trend Radar</em></h1><p>{t.subtitle}</p><div className="hero-metrics" aria-label="Radar coverage"><span><b>{data.repoCount}</b> repositories</span><span><b>{data.historyDates.length}</b> snapshots</span><span><b>{data.updatedAt}</b> latest signal</span></div></div><div className="header-tools"><div className="language-switch" aria-label={t.language}><button className={language === 'zh' ? 'active' : ''} onClick={() => setLanguage('zh')}>繁中</button><button className={language === 'en' ? 'active' : ''} onClick={() => setLanguage('en')}>EN</button></div><div className="updated"><span>{t.updated}</span><strong>{data.updatedAt}</strong></div></div></header>
    ...
    <section className="dashboard-grid"><section className="table-panel"><div className="panel-heading"><div><h2>{t.momentum(rangeLabel)}</h2><p>{t.coverage(coverageStart, coverageEnd)}</p></div><span className="result-count">{t.count(filtered.length, data.repoCount)}</span></div><div className="table-wrap"><table><thead><tr><th>#</th><th>{t.repo}</th><th>{t.purpose}</th><th>{t.delta(rangeLabel)}</th><th>{t.stars}</th><th>{t.updatedOn}</th><th>{t.trend}</th><th>{t.risk}</th></tr></thead><tbody>{pageRows.map((row, index) => <tr key={row.name} className={selected.name === row.name ? 'selected' : ''} onClick={() => openDetail(row.name)}><td className="rank">{(currentPage - 1) * pageSize + index + 1}</td><td><strong>{row.name}</strong><div className="row-tags">{row.isNew && <span className="tag selected">{t.newEntry}</span>}{row.trendingStarsToday && <span className="tag automation">{t.trendingToday(number(language).format(row.trendingStarsToday))}</span>}{row.tags.slice(0, 3).map((tag) => <span key={tag} className={`tag ${tag.toLowerCase()}`}>{tagText[tag]?.[language] || tag}</span>)}</div></td><td className="description"><span>{language === 'zh' ? row.descriptionZh : row.description || t.noDescription}</span>{language === 'zh' && <small>{t.original}</small>}</td><td className="delta">{row.isNew ? '—' : `+${number(language).format(row.windowDelta)}`}</td><td>{number(language).format(row.stars)}</td><td className="updated-date">{row.pushedAt || '—'}</td><td><Sparkline series={row.windowSeries} /></td><td><span className={row.category === 'Watch' ? 'risk warning' : 'risk'}>{row.category === 'Watch' ? t.review : row.license}</span></td></tr>)}</tbody></table></div>{pageCount > 1 && <nav className="pagination" aria-label="Pagination"><button onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={currentPage === 1}>{t.previousPage}</button>{Array.from({ length: pageCount }, (_, index) => index + 1).map((pageNumber) => <button key={pageNumber} className={pageNumber === currentPage ? 'active' : ''} onClick={() => setPage(pageNumber)} aria-current={pageNumber === currentPage ? 'page' : undefined}>{pageNumber}</button>)}<button onClick={() => setPage((current) => Math.min(pageCount, current + 1))} disabled={currentPage === pageCount}>{t.nextPage}</button><span>{t.page(currentPage, pageCount)}</span></nav>}</section><section className="right-rail"><InsightPanel records={filtered.length ? filtered : decorated} tagOptions={tagOptions} language={language} /><TrendChart records={filtered.length ? filtered : decorated} rangeLabel={rangeLabel} language={language} /><div className="category-panel"><div className="panel-heading"><h2>{t.categoryDistribution}</h2><span>{t.byRepos}</span></div>{categoryCounts.map((item) => <div className="distribution" key={item.key}><span>{categoryText[item.key][language]}</span><div><i style={{ width: `${item.count / maxCount * 100}%` }} /></div><b>{item.count}</b></div>)}</div></section></section>
