...
function App() {
  ...
  return <main className="app-shell"><a className="skip-link" href="#radar">Skip to radar</a><aside className="sidebar"><div className="brand-lockup" aria-label="metabiz GitHub AI Trend Radar"><img src={`${import.meta.env.BASE_URL}metabiz-gold-icon-transparent.png`} alt="metabiz" /><div><strong>metabiz</strong><span>INTELLIGENCE</span></div></div><div className="side-status"><strong>GitHub AI Radar</strong><span>DAILY SIGNAL · {data.updatedAt}</span></div><p className="source">{t.sourceShort}</p></aside><section className="workspace" id="radar"><header><div className="hero-copy"><span className="eyebrow">metabiz intelligence / OPEN-SOURCE SIGNAL</span><h1>GitHub AI <em>Trend Radar</em></h1><p>{t.subtitle}</p><div className="hero-metrics" aria-label="Radar coverage"><span><b>{data.repoCount}</b> repositories</span><span><b>{data.historyDates.length}</b> snapshots</span><span><b>{data.updatedAt}</b> latest signal</span></div></div><div className="header-tools"><div className="language-switch" aria-label={t.language}><button className={language === 'zh' ? 'active' : ''} onClick={() => setLanguage('zh')}>繁中</button><button className={language === 'en' ? 'active' : ''} onClick={() => setLanguage('en')}>EN</button></div><div className="updated"><span>{t.updated}</span><strong>{data.updatedAt}</strong></div></div></header>
