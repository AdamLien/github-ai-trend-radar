...
本日最值得投入「可驗證試作」的不是單純高星專案，而是把 AI coding agent 的工作流程、知識脈絡與 MCP 介面接起來的三條線：`addyosmani/agent-skills`／`obra/superpowers`（工程方法與 skills）、`Graphify-Labs/graphify`（可解釋的程式碼知識圖譜）、`GLips/Figma-Context-MCP`（設計到程式的上下文）。
...
- 搜尋：10 組 AI／MCP／skills／agent／RAG／wiki／developer automation 查詢，每組最多 10 筆，去重後 89 repo。
- 動能：25/89 的 `pushed_at` 落在 8/7 UTC；沒有前一日可比集合，因此不以總 stars 排名，也不假造日增星。
